...
% 아날로그 체비셰프 I형 필터 + 임펄스 불변 변환
wp_a=wp_d*fs; % 아날로그 통과대역 경계주파수
ws_a=ws_d*fs; % 아날로그 저지대역 경계주파수
[N,wc]=cheb1ord(wp_a,ws_a,Ap,As,'s'); % 필터 차수 및 차단 주파수 계산
[num_a,den_a]=cheby1(N,Ap,wc,'s'); % 아날로그 필터 전달 함수 H(s) 계산
[num_d,den_d]=impinvar(num_a,den_a); % 디지털 필터 전달 함수 H(z) 계산
[Hz,w]=freqz(num_d,den_d); % 필터 주파수 응답 계산
% 아날로그 체비셰프 I형 필터 + 쌍선형 변환
wp=(2/Ts)*tan(wp_d/2); % 통과 대역 경계 주파수 미리 휨
ws=(2/Ts)*tan(ws_d/2); % 저지 대역 경계 주파수 미리 휨
[N2,wc2]=cheb1ord(wp,ws,Ap,As,'s'); % 필터 차수 및 차단 주파수 계산
[num_a2,den_a2]=cheby1(N2,Ap,wc2,'s'); % 아날로그 필터 전달 함수 H(s) 계산
